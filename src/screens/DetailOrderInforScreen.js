...
export const DetailOrderInforScreen = () => {
  ...
  const handleOrder = () => {
    if (!inputs.name || !inputs.phone || !inputs.email) {
      setErrorMessage(
        "Please fill all information fields before placing an order."
      );
      return;
    }
    if (isError.name || isError.phone || isError.email) {
      setErrorMessage(
        "Please fill all valid information before placing an order."
      );
      return;
    }
    if (!checked) {
  ...
  return (
    <SubLayout title={"Personal Information"} goBackButton={true}>
      ...
      <ScrollView className="mt-5">
        ...
        <View className="px-5 mb-4">
          <View className="flex-row">
            <Text className="mb-2 mr-1 text-white font-semibold">
              Phone number
            </Text>
        ...
        <View className="flex-row justify-center items-center flex-wrap">
          ...
          <Text className="text-white">
            I agree to receive notifications order tickets via email.
          </Text>
          <Text className="text-red-500">*</Text>
        </View>
